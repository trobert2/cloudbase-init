...
class SetUserPasswordPluginTest(unittest.TestCase):
    ...
    def test_set_password(self):
        services_base.BaseMetadataService.can_post_password = True
        CONF.username = 'Admin'

        m = services_base.BaseMetadataService.is_password_set(self.version)
        m.AndReturn(False)

        m = windows.WindowsUtils.user_exists(CONF.username)
        m.AndReturn(True)

        m = windows.WindowsUtils.generate_random_password(14)
        ...
        windows.WindowsUtils.set_user_password(CONF.username,
                                               self.password)
    ...
    def test_cannot_post_password(self):
        CONF.username = 'Admin'
        services_base.BaseMetadataService.can_post_password = False
