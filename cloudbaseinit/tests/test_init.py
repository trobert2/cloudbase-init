...
class InitManagerTest(unittest.TestCase):
    ...
    def _test_config_host(self, reg_instance, reg_status=None):
        m = windows.WindowsUtils.wait_for_boot_completion()
        m.AndReturn(None)

        m = plugin_factory.PluginFactory.load_plugins()
        m.AndReturn(CONF.plugins)

        m = metadata_factory.MetadataServiceFactory.get_metadata_service()
        m.AndReturn(self.service)

        m = windows.WindowsUtils.get_config_value(mox.IsA(str), mox.IsA(str))
